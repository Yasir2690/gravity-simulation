...
const int MAX_HISTORY = 600; // 10 seconds at 60fps
        ...
        if (showTrails) {
            ...
            if (rainbowTrails) {
                ...
                newTrail.color = Color(
                    (int)(sin(hue * 3.14159/180) * 127 + 128),
                    (int)(sin((hue + 120) * 3.14159/180) * 127 + 128),
                    (int)(sin((hue + 240) * 3.14159/180) * 127 + 128)
                );
            } else {
                newTrail.color = color;
            }
...
int main() {
    ...
    RectangleShape colorPickerPreview;
    colorPickerPreview.setSize(Vector2f(30, 30));
    ...
    while (window.isOpen()) {
        ...
        while (const optional<Event> event = window.pollEvent()) {
            ...
            if (const auto* keyPressed = event->getIf<Event::KeyPressed>()) {
                ...
                if (keyPressed->code == Keyboard::Key::R) rainbowTrails = !rainbowTrails;
                if (keyPressed->code == Keyboard::Key::F) followMode = !followMode;
                if (keyPressed->code == Keyboard::Key::X && !history.empty()) rewind();
                ...
                if (keyPressed->code >= Keyboard::Key::Num0 && keyPressed->code <= Keyboard::Key::Num9 && followMode) {
                    int num = keyPressed->code - Keyboard::Key::Num0;
            ...
            if (const auto* wheelScrolled = event->getIf<Event::MouseWheelScrolled>()) {
                if (wheelScrolled->wheel == Mouse::Wheel::Vertical) {
                    wheelScrolled->delta > 0 ? view.zoom(0.9f) : view.zoom(1.1f);
                    window.setView(view);
                }
            }
        }
        ...
        if (showUI) {
            RectangleShape uiBg(Vector2f(320, 550));
            ...
            ss << "Rainbow: " << (rainbowTrails ? "ON" : "OFF") << " (R)\n";
            ss << "C: Color Pick | N: Rename\n";
            ss << "Q: Add Comet | F: Follow Mode\n";
            ss << "X: Rewind | Up/Down: Speed\n";
            ...
            if (followMode) ss << "Following: " << bodies[followIndex].name << "\n";
            if (colorPickMode) ss << "COLOR PICK MODE ACTIVE\n";
            if (renamingMode) ss << "Rename: " << newName << "_ (Press Enter)\n";
            ...
            if (!energyHistory.empty() && !velocityHistory.empty()) {
                RectangleShape graphBg(Vector2f(300, 100));
                graphBg.setFillColor(Color(0, 0, 0, 180));
                graphBg.setPosition(Vector2f(10, 440));
                window.draw(graphBg);
                
                for (size_t i = 1; i < energyHistory.size(); i++) {
                    Vertex line[] = {
                        Vertex(Vector2f(10 + i * 300.0f / energyHistory.size(), 540 - energyHistory[i-1] / 1000)),
                        Vertex(Vector2f(10 + (i+1) * 300.0f / energyHistory.size(), 540 - energyHistory[i] / 1000))
                    };
        ...
        if (colorPickMode && selectedPlanet >= 0) {
            ...
            for (int i = 0; i < 8; i++) {
                ...
                if (Mouse::isButtonPressed(Mouse::Button::Left)) {
                    ...
                    if (mousePos.x > 810 + (i%4)*45 && mousePos.x < 850 + (i%4)*45 &&
                        mousePos.y > 450 + (i/4)*45 && mousePos.y < 490 + (i/4)*45) {
                        bodies[selectedPlanet].color = colors[i];
                        colorPickMode = false;
                        selectedPlanet = -1;
                    }
                }
